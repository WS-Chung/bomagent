    ...
    >
      <div className="rounded-2xl glass p-4 sm:p-5 space-y-3">
        ...
        <FlowDiagram
          steps={[
            ...
            {
              title: "STEP 2 · 각 부품의 재고 · 단가 · L/T · MOQ를 조회",
            ...
            },
            {
              ...
              body: "vlookup · sumif · index/match 체인이 여러 시트를 오간다. 시트 하나가 이름을 바꾸면 참조가 깨진다. 실무자는 매번 링크 무결성을 확인한다.",
            },
            {
              title: "STEP 5 · 위험도 판단은 담당자의 감(感)으로",
              body: "L/T가 12주인 부품과 4주인 부품의 우선순위가 개인 판단에 좌우된다. 담당자 교체 시 판단 기준이 함께 바뀐다.",
            },
            {
              title: "STEP 6 · 공급사별로 수기로 발주서 작성",
              body: "엑셀 발주서 템플릿을 복사해 부품 리스트를 붙여넣고 담당자·연락처·사업자번호를 다시 입력한다. 오타 · 누락 · 잘못된 공급사 매핑이 발생.",
            },
            {
              title: "STEP 7 · 각 부서에 문서 재공유 · 재확인",
              body: "회계·생산·QC에 최신본을 다시 전달한다. 다음 주 회의에서 또 어긋난 숫자가 발견되어 처음 단계로 돌아간다.",
            },
          ]}
        />
      </div>
      ...
      <div className="rounded-2xl glass p-4 sm:p-5 space-y-3">
        <p className="text-[12px] uppercase tracking-[0.18em] text-emerald-300 font-semibold">
          🟢 To-Be · BOM Agent 워크플로우 (한 명 · 30분)
        </p>
        <FlowDiagram
          steps={[
            {
              title: "STEP 1 · 재고관리 페이지에서 신규 부품 등록",
              body: "Prefix만 선택하면 Self PN이 자동 채번된다. 분류·제조사·공급사는 드롭다운에서 &quot;+ 신규 추가&quot;로 인라인 생성. 입고 즉시 재고에 반영.",
            },
            {
              title: "STEP 2 · 양산계획 매트릭스에 월별 수량 입력",
              body: "제품 × 월 매트릭스에 수량을 채우고 [저장] 버튼 한 번. Sequential PATCH로 락 경쟁 없이 원자적 반영. 지난 월은 자동 숨김.",
            },
            {
              title: "STEP 3 · BOM 편집 모달에서 라인 구성",
              body: "제품 행의 [BOM] 링크 클릭 → 부품 선택 · pt · Reference 입력. 자유 텍스트 입력은 금지되어 있어 미등록 부품이 흘러들 수 없다.",
            },
            {
              title: "STEP 4 · 대시보드에서 자동 계산 결과 확인",
              body: "저장과 동시에 Forecast Engine이 재계산되어 2M/6M-FCST · 위험도 · 추천 발주량이 즉시 갱신된다. 결과는 위험도 → 6M-FCST 오름차순 정렬.",
            },
            {
              title: "STEP 5 · 위험도·공급사 필터로 발주 후보 좁히기",
              body: "RED만 필터하면 즉시 발주 필요 부품, YELLOW를 함께 보면 계획적 발주 후보. 특정 공급사 선택 시 우상단 [발주서 자동작성] 버튼이 활성화된다.",
            },
            {
              title: "STEP 6 · 공급사별 발주서 초안 자동 생성",
              body: "A4 세로 포맷 발주서에 공급사 상세정보(정식명·담당자·이메일·사업자번호) · 부품 리스트 · 추천 수량 · 단가가 자동 채워진다. [인쇄]로 PDF 저장.",
            },
            {
              title: "STEP 7 · 감사 로그로 모든 변경 이력이 자동 축적",
              body: "누가 언제 어떤 값을 어떻게 바꿨는지 컬럼당 1건씩 기록된다. 회의 · 재공유 · 문서 대조 없이 시스템이 SSOT를 유지한다.",
            },
          ]}
        />
      </div>
      ...
      <div className="rounded-2xl glass p-4 sm:p-5 space-y-3">
          ...
          <table className="w-full text-[14px]">
            ...
            <tbody className="text-slate-200">
              ...
              <tr>
                ...
                <td className="py-2.5 px-3.5 text-emerald-300">Audit Log 100%</td>
        ...
        <p className="text-[12px] text-slate-400">
          위 수치는 60~80종 부품·4개 제품 규모의 중소 PCB 양산 업체를 가정한
          추정치이며, 실제 도입 시 <b>파일럿 프로젝트로 정량화</b>가 필요하다.
        </p>
      </div>
    </SlideShell>
  );
